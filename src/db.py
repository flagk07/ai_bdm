...
class Database:
	...
	def is_allowed(self, tg_id: int) -> bool:
		try:
			res = self.client.table("allowed_users").select("tg_id, active").eq("tg_id", tg_id).single().execute()
			row = (res.data or {})
	...
	def get_or_register_employee(self, tg_id: int) -> Optional[Employee]:
		try:
			existing = self.client.table("employees").select("tg_id, agent_name, active").eq("tg_id", tg_id).maybe_single().execute()
			row = existing.data
			if not row:
				inserted = self.client.table("employees").insert({"tg_id": tg_id}).select("tg_id, agent_name, active").single().execute()
				row = inserted.data
			return Employee(tg_id=row["tg_id"], agent_name=row["agent_name"], active=row.get("active", True))
	...
	def save_attempts(self, tg_id: int, attempts: Dict[str, int], for_date: date) -> None:
		...
		try:
			...
			# Create employee row if missing
	...
	def month_ranking(self, month_first_day: date, today: date) -> List[Dict[str, Any]]:
		...
		if sums:
			...
			id_to_name = {int(r["tg_id"]): r["agent_name"] for r in (emp.data or [])}
