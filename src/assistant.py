...
def _build_system_prompt(agent_name: str, stats_line: str, group_line: str, notes_preview: str) -> str:
	system = (
		...
		"придумывать точные тарифы/ставки/требования без предоставленной справки. Если вопрос вне рамок — кратко верни диалог к рабочей теме.\n"
		# Данные из бота
		f"Контекст: {stats_line}. {group_line}\n"
		f"Заметки сотрудника:\n{notes_preview}\n"
		# Язык и стиль
		"Стиль: по делу, деловой и доброжелательный, без воды. Короткие абзацы. Если перечисление — нумерованные пункты ‘1.’, ‘2.’ … "
		"Не используй ПДн и не запрашивай их. Если данных не хватает — спроси не больше 1–3 точных уточнений.\n"
		# Формат
		...
		"Одна мысль — один пункт; каждый шаг должен быть исполнимым в заданный период. Связывай советы с метриками (attempts, план/факт, RR).\n"
	)
	return system
